...
function CarsContent() {
  ...
  const searchCars = async () => {
    ...
    try {
      const res = await makeApiRequest(
        `${apiurl.carsSearch}?location=${encodeURIComponent(location)}&pickup_date=${checkIn}&dropoff_date=${checkOut}`,
      );
      if (res.success) {
        setCars(res.data);
      } else {
  ...
  const handleBookCar = async (e: React.FormEvent) => {
    e.preventDefault();
    if (
      !driverForm.first_name ||
      !driverForm.last_name ||
      !driverForm.email ||
            ...
            {loading ? (
              ...
              <div className="text-center py-16 bg-white rounded-2xl border border-gray-100 p-6">
                ...
                <p className="text-gray-400 text-sm">
                  Try searching for a different location or date range
                </p>
              </div>
            ) : (
              cars.map((car) => (
                  ...
                  <div className="flex items-start justify-between border-b border-gray-100 pb-4 mb-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 font-medium">
                        Provided by: {car.rental_company}
                      </p>
                    </div>
              ...
              {bookingCar ? (
                <form onSubmit={handleBookCar} className="space-y-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-2 mb-4 text-xs">
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-500">
                        {bookingCar.car_model}
                      </span>
                      <span className="font-bold text-gray-900">
                        ${Number(bookingCar.price_per_day).toFixed(2)}/day
                      </span>
                    </div>
                    <p className="text-gray-400 uppercase text-[10px] font-semibold">
                      {bookingCar.rental_company} · {bookingCar.car_class}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-500 py-2">
                    ...
                    <span>
                      Driver must present physical license at pick-up.
                    </span>
                  </div>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-400 text-sm">
                    Select a car model to check specifications, enter details
                    and book.
                  </p>
                </div>
              )}
