...
async function searchHotels(params: Record<string, string>): Promise<SearchResult> {
  ...
  if (params.guests) payload.guests = parseInt(params.guests);
  ...
  if (params.bedrooms) payload.bedrooms = parseInt(params.bedrooms);
  if (params.bathrooms) payload.bathrooms = parseInt(params.bathrooms);
  if (params.sort) payload.sort = params.sort;
  if (params.page) payload.page = parseInt(params.page);
...
export default async function SearchPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const { data: properties, total, current_page, last_page } = await searchHotels(params);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-[#0d1637] py-6">
        <SearchBar
          ...
          initialGuests={params.guests ? parseInt(params.guests) : 2}
        />
      </div>
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="global-container px-4 py-3">
          <div className="flex flex-wrap items-center gap-2 text-sm text-gray-600">
            {params.location && (
              <span className="font-semibold text-[#0d1637] text-base">{params.location}</span>
            )}
            ...
            {params.guests && (
              <>
                <span className="text-gray-300">·</span>
                <span>{params.guests} guest{parseInt(params.guests) !== 1 ? 's' : ''}</span>
              </>
            )}
            <span className="ml-auto text-emerald-600 font-semibold">{total} properties found</span>
          </div>
        </div>
      </div>
      ...
      <div className="global-container px-4 py-6">
        <div className="flex gap-6 items-start">
          ...
          <aside className="hidden lg:block w-72 shrink-0 self-start sticky top-24">
            <SearchFiltersClient initialParams={params} key={JSON.stringify(params)} />
          </aside>
