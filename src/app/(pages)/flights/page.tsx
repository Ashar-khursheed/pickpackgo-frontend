...
import { ArrowRight, Plane, ShieldCheck, Ticket, User } from "lucide-react";
  ...
  const searchFlights = async () => {
    ...
    try {
      const res = await makeApiRequest(
        `${apiurl.flightsSearch}?location=${encodeURIComponent(location)}&date=${checkIn}`,
      );
      if (res.success) {
        setFlights(res.data);
      } else {
            ...
            {loading ? (
              ...
              <div className="text-center py-16 bg-white rounded-2xl border border-gray-100 p-6">
                ...
                <p className="text-gray-400 text-sm">
                  Try searching for a different destination or date
                </p>
              </div>
            ) : (
              flights.map((flight) => (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 sticky top-6">
              {bookingFlight ? (
                  ...
                  <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-2 mb-4 text-xs">
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-500">
                        {bookingFlight.airline} {bookingFlight.flight_number}
                      </span>
                      <span className="font-bold text-gray-900">
                        ${Number(bookingFlight.price).toFixed(2)}
                      </span>
                    </div>
                    <p className="text-gray-400">
                      {bookingFlight.departure_airport} →{" "}
                      {bookingFlight.arrival_airport}
                    </p>
                  </div>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-400 text-sm">
                    Select a flight from the list to enter passenger details and
                    book.
                  </p>
                </div>
              )}
